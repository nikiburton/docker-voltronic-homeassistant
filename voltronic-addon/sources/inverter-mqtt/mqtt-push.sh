#!/bin/bash
INFLUX_ENABLED=`cat /opt/inverter-mqtt/mqtt.json | jq '.influx.enabled' -r`
...
pushInfluxData () {
    INFLUX_HOST=`cat /opt/inverter-mqtt/mqtt.json" | jq '.influx.host' -r`
    INFLUX_USERNAME=`cat /opt/inverter-mqtt/mqtt.json" | jq '.influx.username' -r`
    INFLUX_PASSWORD=`cat /opt/inverter-mqtt/mqtt.json" | jq '.influx.password' -r`
    INFLUX_DEVICE=`cat /opt/inverter-mqtt/mqtt.json" | jq '.influx.device' -r`
    INFLUX_PREFIX=`cat /opt/inverter-mqtt/mqtt.json" | jq '.influx.prefix' -r`
    INFLUX_DATABASE=`cat /opt/inverter-mqtt/mqtt.json" | jq '.influx.database' -r`
    INFLUX_MEASUREMENT_NAME=`cat /opt/inverter-mqtt/mqtt.json" | jq '.influx.namingMap.'$1'' -r`
